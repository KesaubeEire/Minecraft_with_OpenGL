...
//本八叉树的策略是：
    1，一次划分所有节点，是满树；
    2，当立方体空间完全包含某物体才剔除,当立方体空间与某物体相交或者完全包含时才查询；
    3，对象放在完全包含它的区域叶子节点内，非根节点不存储对象，默认为物体不可能跨多个叶子节点，都在一个叶子节点的空间范围内部，未考虑交叉的情况
*/
#pragma once
#include <list>
...
class OctreeNode
{
  ...
  public:
    ...
    std::list<T *> GetObjectsAt(float px, float py, float pz, float x_size, float y_size, float z_size);
